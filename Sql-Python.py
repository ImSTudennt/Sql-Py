...
if __name__ == '__main__':
    with psycopg2.connect(database='HW', user='postgres', password='...') as con:
        with con.cursor() as cur:
            drop_table(cur)
            create_db(cur)
            new_client(cur,'Alex','Mosunov','alex6@mail.ru', '79347457766')
            new_client(cur,'Vova','Mosunov','vlad6@mail.ru')
            new_client(cur,'Peter','Meh','peter6@mail.ru', '79347467766')
            add_phone(cur,3,'8937744444')
            change_client(cur,2,'Vladimir', surname='Mosunov')
            change_client(cur,3,phone='89342354454')
            delete_phone(cur, 3, '89342354454')
            delete_client(cur,3)
            find_client(cur,surname='Mosunov')
            con.commit()
    con.close()
